...
def AddParty(request):
    ...
    if request.method=="POST":    
        ...
        dt=PartyModel.objects.create(Code=Code,PartyName=PartyName,Cluster=Cluster,Active=Active,City=City,Branch=Branch,Address=Address,GSTNo=GSTNo,PanNo=PanNo,NO1=NO1,NO2=NO2,NO3=NO3,NO4=NO4,NO5=NO5,NO6=NO6)
        dt.save()
...
def EditParty(request,id):
    ...
    if request.method=="POST":    
        ...
        data.PartyName=request.POST.get('PartyName')
        # data.Cluster=request.POST.get('Cluster')
        did=request.POST.get('Cluster')
        cl= ClusterModel.objects.get(id=did)
        ...
        data.PanNo=request.POST.get('PanNo')
        data.NO1=request.POST.get('NO1')
        data.NO2=request.POST.get('NO2')
        data.NO3=request.POST.get('NO3')
        data.NO4=request.POST.get('NO4')
            ...
            if datein == 'Date':
                ...
                for i in show:
                    ...
                    for j in dt:
                        ...
                        IRCM = InvoiceRCModel.objects.create(InvoiceNo=InvoiceNo,Counter=j.Counter,RCCode=j.RCCode,Unit=j.Unit,HSNCode=j.HSNCode,LCode=i.Code,RCDescription=j.RCDescription,Quantity=j.Quantity,Rate=j.Rate,Labour=j.Labour,Amount=j.Amount,GSTRate=GSTRate,GSTAmount=GSTAmount,GSTTA=GSTTA,TotalAmount=TotalAmount)
                        IRCM.save()
                    GA = GA
                    ...
                    i.IN=InvoiceNo
                    i.save()
                Rc = InvoiceRCModel.objects.filter(InvoiceNo=InvoiceNo)
                Amount = 0
                GSTAmount = 0
                TotalAmount = 0
                for j in Rc:
                ...
                GSTAmount = "%.2f" % GSTAmount
                TotalAmount = "%.2f" % TotalAmount    
                MainInvoice = InvoiceModel.objects.create(PartyName=PartyName,InvoiceData=InvoiceData,InvoiceNo=InvoiceNo,BillMonth=BillMonth,BillYear=BillYear,Tax=Tax,datein=datein,Type=Type,FromDate=FromDate,ToDate=ToDate,Amount=Amount,GSTAmount=GSTAmount,TotalAmount=TotalAmount,Cluster_id=C_id,Cluster=Cluster)
                MainInvoice.save()
                CM = ICountModel.objects.get(Ron='1')
                ...
                GSTAmount = "%.2f" % GSTAmount
                TotalAmount = "%.2f" % TotalAmount
                MainInvoice = InvoiceModel.objects.create(PartyName=PartyName,InvoiceData=InvoiceData,InvoiceNo=InvoiceNo,BillMonth=BillMonth,BillYear=BillYear,Tax=Tax,datein=datein,Type=Type,Amount=Amount,GSTAmount=GSTAmount,TotalAmount=TotalAmount,Cluster_id=C_id,Cluster=Cluster)
                MainInvoice.save()
                CM = ICountModel.objects.get(Ron='1')
                if str(CM.InvoiceNo) == InvoiceNo:
                    CM.InvoiceNo = int(InvoiceNo)+1
...
def AddQuotation(request):
    ...
    if request.method=="POST":
        ...
        dt.save()
        CM=QCountModel.objects.get(id=1)
        if str(CM.QuotationNo) == QuotationNo:
...
def numberToWords(n):
    ...
    def number_to_words(num):
        ...
        result = ""
        if num >= crore:
            result += three(num // crore) + " Crore"
            num %= crore
        if num >= lakh:
            result += " " + three(num // lakh) + " Lakh"
            num %= lakh
        if num >= thousand:
            result += " " + three(num // thousand) + " Thousand"
            num %= thousand
        if num:
            result += " " + three(num)
    ...
    if isinstance(n, float):
        ...
        if fractional_part > 0:
            ...
            fractional_words = number_to_words(int(round(fractional_part * 100)))
...
def TaxInvoice(request,id):
    ...
    Amount = "%.2f" % Amount
    GSTAmount = "%.2f" % GSTAmount
    TotalAmount3 = "%.2f" % TotalAmount
    Amount = eval(Amount)
    GSTAmount = eval(GSTAmount)
    TotalAmount = eval(TotalAmount3)
    ...
    for i in party:
        ...
        break
    # Test cases
    TotalAmount2=numberToWords(TotalAmount2)
    main={'dt':dt,'party':party,'p':p,'Rc':Rc,'pd':pd,'py':py,'Amount':Amount,'InvoiceNo':InvoiceNo,'GSTAmount':GSTAmount,'TotalAmount':TotalAmount,'TotalAmount2':TotalAmount2}
...
@login_required(login_url='Login')
def WInvoice(request):
    dt=InvoiceModel.objects.all()
    data={'dt':dt}
    return render(request,'winvoice.html',data)

@login_required(login_url='Login')
def AddWInvoice(request):
    # ICountModel.objects.all().delete()
    try:
        CM=ICountModel.objects.get(Ron='1')
        InvoiceNo = CM.InvoiceNo
    ...
    if request.method=="POST":    
        ...
        if datein == 'Date':
            ...
            C_id=int(C_id)
            maindata={"PN":PN,'InvoiceNo':InvoiceNo,'show':show,'PartyName':PartyName,'Type':Type,'datein':datein,'ttl':ttl,'C_id':C_id,'Cluster':Cluster}
    return render(request,'addwinvoice.html',maindata)

@login_required(login_url='Login')
def AddWInvoiceMaIN(request):
    if request.method=="POST":
        InvoiceData=request.POST.get('InvoiceData')
        InvoiceNo=request.POST.get('InvoiceNo')
        BillMonth=request.POST.get('BillMonth')
            ...
            if datein == 'Date':
                ...
                for i in show:
                    ...
                    for j in Rc:
                        ...
                        TotalAmount += eval(j.TotalAmount)
                    Amount = "%.2f" % Amount
                    GSTAmount = GSTAmount * 2 if Tax == 'GST' else GSTAmount
                    GSTAmount = "%.2f" % GSTAmount
                    TotalAmount = "%.2f" % TotalAmount
                    CM = ICountModel.objects.get(Ron='1')
                    InvoiceNo = CM.InvoiceNo
                    ...
                    MainInvoice.save()
                    CM = ICountModel.objects.get(Ron='1')
                    CM.InvoiceNo = int(CM.InvoiceNo)+1
                    CM.save()
                return redirect("/AddWInvoice")
            else:
                show=EntryModel.objects.filter(PartyName=PartyName,CallType=Type,complet='0',Cluster_id=C_id)
                for i in show:
                    dt = RCModel.objects.filter(Counter=i.Counter)
